...
func (s *state) round5() (tss.StateMachine, []tss.Message, error) {
	...
	si, ok := s.tempData["si"].(*big.Int)
	if !ok || si == nil {
		return nil, nil, fmt.Errorf("missing local signature share")
	}
	...
	r, ok := s.tempData["r"].(*big.Int)
	if !ok || r == nil {
		return nil, nil, fmt.Errorf("missing signature R value")
	}
